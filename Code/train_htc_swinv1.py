###  Train an Hybrid Task Cascade (HTC) with SWIN V1 using all datasets
...
for step in range(nb_it):
    ...
    for fold in path_iterator:
        ...
        # Load config file
        cfg = Config.fromfile('Skinet/Projet/my_configs/used_configs/htc_swinv1.py')
        ...
        # See list_pretrained_swin v1.txt
        pretrained_swin = 'https://github.com/SwinTransformer/storage/releases/download/v1.0.2/cascade_mask_rcnn_swin_base_patch4_window7.pth'
        cfg.model.backbone.init_cfg = dict(checkpoint=pretrained_swin, type='Pretrained')
        cfg.pretrained = None

        # Swin parameters (from mmpretrain tool https://github.com/open-mmlab/mmpretrain/blob/main/mmpretrain/models/backbones/swin_transformer.py)
        # Custom import
        ...
        cfg.model.backbone.type='mmpretrain.models.SwinTransformer'
        ...
        self = SwinTransformer(arch=arch, out_indices=(0,1,2,3), 
        ...
        cfg.work_dir = f'Skinet/Projet/exps/htc_swin_v1_all_data/step{step+1}/{mode}'
        prev_train_path = f'Skinet/Projet/exps/htc_swin_v1_all_data/step{step+1}/{mode}'
        ...
        config='Skinet/Projet/my_configs/used_configs/htc_swinv1.py'
